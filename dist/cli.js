...
var argv = yargs_default(hideBin(process.argv)).command("bundle", "Bundles the handlers", {
  ...
  const bundledConfigPath = path2.resolve(cwd, "hdb-proxy.json");
  const bundledConfig = {};
  ...
  Object.entries(handlers).forEach(([key, handler]) => {
    ...
    const outPath = path2.join(out, `${key}.mjs`);
    console.log(`Bundling ${key} to ${outPath}...`);
    const p = child_process.spawnSync("bun", ["build", handlerPath, "--target", "node", "--format", "esm", "--outfile", outPath], {
      cwd,
      stdio: "inherit"
    });
    if (p.status !== 0) {
  ...
  fs2.writeFileSync(bundledConfigPath, JSON.stringify(bundledConfig, null, 2));
}).help().argv;
